...
public class ControlPanelController implements Initializable {
	...
	private void getInput(int[] pureInputSize, int[] pureBlockSize) throws Exception {
		if (inputSizeField.getText().trim().isEmpty() || blockSizeField.getText().trim().isEmpty()) {
			throw new Exception("Input size is empty or block size is empty, " + "please specify a integral value.");
		}
		try {
			if ((pureBlockSize[0] = getBlockSize()) > getInputSize() || getBlockSize() < 1
					|| (pureInputSize[0] = getInputSize()) < 3) {
				throw new Exception("Invalid input, the block size is either "
						+ "greater than the input size or is less than 1 or " + "the input size is less than 3.");
			}
		} catch (NumberFormatException ex) {
			throw new Exception(ex.getMessage() + "\nYou MUST provide an " + "integral value.");
		}
