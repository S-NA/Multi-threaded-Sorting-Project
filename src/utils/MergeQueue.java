...
public class MergeQueue<T extends Comparable<? super T>> {
    ...
    public void execute() {
        ...
        while (sortedArrays.size() > 1) {
            ...
            /* Leverages how a queue data structure works.
             * Even though parallelism may misorder things the times that
             * happens is substantially less than the benefit we gain from it.
             * inputSize := 100, blockSize := 33
             * with parallism 6ms
             * with loop 16ms
             */
...
//            System.out.println("Actual Workers: " + mWorkers.size() + " " + "Capacity: " + getCapacity((ArrayList<?>) mWorkers)); /* Test. */
            mWorkers.clear();
            threadPool.clear();
        }
    }
